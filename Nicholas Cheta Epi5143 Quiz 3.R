...
dim(mpg)
#[1] 234 11
#There are 234 observations and 11 variables in the mpg dataset.
...
ggplot(data=mpg)+geom_point(mapping = aes(x=displ, y=hwy, colour=class, shape = drv, size=cyl))

# Question 5. Modify the code to add a suitable title of your choice to your plot
plot2 <- ggplot(data=mpg)+geom_point(mapping = aes(x=displ, y=hwy, colour=class, shape = drv, size=cyl))
plot2+labs(title='Highway Fuel Efficiency vs. Engine Size')
